...
const PAYMENT_CONFIG = {
    ...
    MERCHANT_KEY: 'BRtzj6c0dIpYuFQnFVlWqHFZRvvf1nkc',  // 商户密钥（KEY）
    
    // 支付相关配置
    RETURN_URL: window.location.origin + '/payment-success.html',  // 支付成功后的跳转地址
    NOTIFY_URL: 'https://penally-pickier-anson.ngrok-free.dev/api/payment-notify',  // 异步通知地址
    
    // 创建支付订单
    createOrder: function(orderInfo) {
        const { amount, orderNo, productName } = orderInfo;
        
        // 确保money参数格式正确（保留两位小数）
        ...
        // 添加密钥（不带key=前缀）
        const signStr = prestr + this.MERCHANT_KEY;
        console.log('正在生成签名...');
        
        // 计算MD5签名并转为小写
        params.sign = this.md5(signStr).toLowerCase();
